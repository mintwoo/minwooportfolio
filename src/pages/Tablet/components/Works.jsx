				...
				<S.List>
					{WORKS_ITEMS.map((item, index) => {
						return item.map((item2, index2) => {
							if (index === 0 ? index2 % 2 === 0 : (WORKS_ITEMS[index - 1].length + index2) % 2 === 0) {
								return (
									<S.ListItem
										key={item2.title}
										{...animatedItem[index === 0 ? index2 : WORKS_ITEMS[index - 1].length + index2]}
									>
										<S.ItemImage image={item2.image} index={'0.5rem 0 0 0.5rem'} />
										<S.TextContainer>
								...
								return (
									...
									>
										...
										<S.ItemImage image={item2.image} index={'0 0.5rem 0.5rem 0 '} />
									</S.ListItem>
								);
							}
						});
